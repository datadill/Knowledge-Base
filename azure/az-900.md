...
* Each region (e.g. east-us, west-us) will have multiple physical buildings that are relatively close together, but have separate cooling/networking/power/etc (Availability Zone)
  ...
  * Zone redundant means that the service spans the availability zones automatically (native).. other services have to be pinned to each individual zones (e.g. a VM has to be created and applied to each AZ manually)
